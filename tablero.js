...
//marmoset.embed('Tecladoview.mview', { width: 500, height: 300, autoStart: true, fullFrame: false, pagePreset: false } );
...
function setup() { // create all the objects 0 = dot, 1 =artist
  ...
  webforceSize = createSlider(5, windowHeight / 2 + 300, 5);
  ...
  for (i = 0; i < 1676; i++) { // pushing dots at json position
    dotArray.push(new Dot(height / 2, height / 2, imgdots0[i].weight, 0, [], i));
...
function interpolateDots() { // step number (between 0 and 1)
  if (updatingdots) {
    ...
    for (i = 0; i < dotArray.length; i++) {
      dotArray[i].target = interpolateVec(dotArray[i].target, tempArrayPos[i], 0.3);
...
function Dot(x, y, weight, family, tags, id, name) { //recieve pos, weight, family and tags
  ...
  this.displayartist = function() {
    ...
    } else if (this.distance < 40 && (frameCount > saveframe + 80)) { // preview names 80 frames after sceneActivated
      fill(255, 88, 30);
...
function mousePressed() {
  if (!sceneActivated) {
    updatedotArray(imgdots0);
    sceneActivated = true;
    webforceSize.value(windowHeight / 2);
