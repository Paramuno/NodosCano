...
function setup() { // create all the objects and oscillators //families: 1=dot 2=elementartist 3=elementinterviews 4=elementvideos 5= elementscans
  ...
  for (let i = 0; i < 19; i++) { // creating artist dots
    ...
    artistArray.push(new Dot(random((width / 2) - 1, (width / 2) + 1), random((height / 2) - 1, (height / 2) + 1), currWeight, 1, tags[i], i));
...
function Dot(x, y, weight, family, tags, id) { //recieve pos, weight, family and tags
  ...
  this.display = function() {
    ...
    if (this.family === 0) {
      ...
      for (let particleA = 0; particleA < artistArray.length; particleA++) {
        let accelerationX = 0,
          accelerationY = 0;
      ...
      this.location.x += velocityX[this.id];
      this.location.y += velocityY[this.id];
      velocityX[this.id] += random(-0.1, 0.1);
      velocityY[this.id] += random(-0.1, 0.1);
